 ...
 * @author Thisura
 */
public class Receptionist_creation_Model {
    ...
    public Receptionist_creation_Model(){
          con = Utility_Class_connection.connect();
          
    }
    public Receptionist_creation_Model(JTable D_ReceptionistTable_view, JLabel D_receptionist_id_label){
          con = Utility_Class_connection.connect();
    ...
    public void insertValues(String DfName, String DlName, String DuserName, String Dpassword, String DreEnterPass, String DvalidPass,
                             String DcontractNum, String DeMail, String DRecepNum) throws SQLException{
        
        pst = con.prepareStatement("insert into receptionist(receptionist_no,first_name,last_name,username,password,contract_number,email)values(?,?,?,?,?,?,?)");
               pst.setString(1,DRecepNum); 
    ...
    public void updateValues(String DfName, String DlName, String DuserName, String Dpassword, String DreEnterPass, String DvalidPass,
        ...
        pst = con.prepareStatement("update receptionist set first_name = ?,last_name = ?,username = ?,password = ?,contract_number = ?,email = ? where receptionist_no = ?");
    ...
    public void deleteValues(JLabel D_receptionist_id_label){
        ...
        try {  
            pst = con.prepareStatement("delete from receptionist where receptionist_no = ?");
            pst.setString(1, D_receptionist_id_label.getText());
            pst.executeUpdate();
            
        } catch (SQLException ex) {
            Logger.getLogger(Receptionist_creation_Model.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        try {
            String sql = "SELECT * FROM receptionist WHERE username LIKE'%"+searchT+"%'";
    ...
    public void receptionist_table(){
        try {
            ...
            while(rs.next()){
                ...
                for(int i = 1; i < c; i++){
                     ...
                     v1.add(rs.getString("receptionist_no"));
    ...
    public void autoAID(){
        ...
        try {
            ...
            rs = s.executeQuery("select MAX(receptionist_no) from receptionist");
            rs.next();
            rs.getString("MAX(receptionist_no)");
            
            if( rs.getString("MAX(receptionist_no)") == null){
                
                D_receptionist_id_label.setText("RT001");
            }
            else{
                
                long id = Long.parseLong(rs.getString("MAX(receptionist_no)").substring(2,rs.getString("MAX(receptionist_no)").length()));
                id++;
                 D_receptionist_id_label.setText("RT" + String.format("%03d", id));
            }
